      ...
      <div className="relative overflow-hidden group border transition-all duration-400 border-[#2B2B2B]">
        ...
        {item.type === "image" ? (
          <img
            ...
            className={`w-full h-[500px] object-cover transition-all duration-400 ease-out group-hover:scale-105 ${
              item.scale || ""
            }`}
          />
        ) : (
          <video
            ...
            muted
            playsInline
            className={`w-full h-[500px] object-cover transition-all duration-400 ease-out ${
        ...
        <div className="absolute inset-0 flex">
          <div className="translate-y-full group-hover:translate-y-0 group-hover:bg-black/30 group-hover:backdrop-blur-xs transition-all duration-400 ease-out py-20 px-12">
            <p className="text-xl leading-[27px] text-[#A1A1A1]">
              {item.description}
            </p>
          </div>
        </div>
      </div>
      <p className="text-[22px] leading-[27px] font-medium">{item.title}</p>
    </div>
  );

  return (
    <div className="py-20 pb-36 min-h-screen bg-black text-white">
      {/* Tabs */}
      <div
        className="flex flex-wrap gap-3 pb-20 justify-start px-4 md:px-10 lg:px-16"
        style={{
      ...
      >
        {tabs.map((tab) => (
          <button
            ...
            className={`px-8 py-1 rounded-full border transition-all duration-300 text-sm leading-[27px]
              ${
                activeTab === tab
      ...
      {/* Dynamic Content */}
      {current && (
        <div className="pt-12 flex flex-col gap-16 relative">
          {/* Hero Section */}
          <div className="grid grid-cols-2 items-center overflow-hidden px-4 md:px-10 lg:px-16">
            <div className="flex flex-col space-y-6">
              <h2
                className="text-[64px] leading-[106px]"
                style={{
                  background:
              ...
              <p className="text-xl text-[#A1A1A1] leading-[27px] relative z-10 w-full max-w-[92%]">
                {current.desc}
              </p>
            </div>
              ...
              {activeTab === "UX/UI Designing" ? (
                <div className="relative">
                  <img
                    src={current.video}
                    alt={current.title}
                    className="h-[580px] object-cover rotate-[30deg]"
                  />
                  <img
                    src={UXImage1}
                    alt=""
                  ...
                  <img
                    src={UXImage2}
                    alt=""
                    className="absolute bottom-0 right-0 w-[110px]"
                  />
                </div>
          ...
          >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
                  {current.marketingSections.map((section) => (
                    <div key={section.key} className="space-y-6">
                      {/* Slideshow */}
                      <div className="relative overflow-hidden h-[510px]">
                      ...
                      {/* Collapsible Section */}
                      <div className="px-3 space-y-8 transition-all duration-500">
                        <div
                          className="flex justify-between items-center cursor-pointer select-none"
                          onClick={() => toggleMarketingSection(section.key)}
                        >
                          <p className="text-2xl leading-[42px] font-normal">
                            {section.title}
                          </p>
                          <IoIosArrowDown
                        ...
                        >
                          <p className="text-base leading-[27px] text-[#A1A1A1] text-justify h-[170px]">
                            {section.description}
                          </p>
                        </div>
                      </div>
            ...
            {activeTab !== "Digital Marketing" &&
              activeTab !== "Affiliate Partnership" && (
                <div className="grid lg:grid-cols-3 gap-6 sm:gap-8 md:gap-16  py-32 relative">
                  <div className="flex flex-col gap-14 col-span-1 w-full max-w-[420px]">
                    <h2
                      className="text-[32px] leading-[42px] flex flex-col"
                      style={{
                        background:
                          "linear-gradient(180deg, #FFFFFF 30.38%, #000000 130.49%)",
                        WebkitBackgroundClip: "text",
                    ...
                    <p className="text-[#A1A1A1] text-xl leading-[27px]">
                      {current.whatMakes.desc}
                    </p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 col-span-2 gap-3 md:gap-8 items-start">
                    {current.features.map((feature, i) => (
                      <div
                        key={i}
                        className={`bg-[#151515] rounded-[10px] p-8 py-12 transition-all h-full group hover:bg-[#FBCA0C] hover:text-black duration-500 ease-in-out ${
                          i % 2 === 0
                            ? "hover:-rotate-[3deg]"
                            : "hover:rotate-[3deg]"
                        }`}
                      >
                        <div className="h-full flex flex-col items-start">
                          <img
                            src={feature.img}
                            alt={feature.title}
                            className="mb-10 h-[65px] group-hover:invert duration-300 ease-in-out"
                          />
                          <h3 className="text-[22px] leading-[27px] font-medium mb-12 h-[40px]">
                            {feature.title}
                          </h3>
          ...
          {/* FAQ Section */}
          <div className="grid lg:grid-cols-2 px-4 md:px-10 lg:px-16 relative">
            <div className="flex flex-col gap-14 w-full max-w-[650px]">
              <h2
                className="text-[48px] leading-[58px] flex flex-col"
                style={{
                  background:
                    "linear-gradient(180deg, #FFFFFF 30.38%, #000000 130.49%)",
                  WebkitBackgroundClip: "text",
              ...
              <div className="border border-[#363636] rounded-[10px] p-8 space-y-8">
                <h2 className="text-[22px] leading-[42px]">
                  Still Have a Question?
                </h2>
                <p className="text-[#A1A1A1] text-base leading-[27px]">
                ...
                <button className="group bg-[#FBCA0C] text-black text-xs font-normal leading-[23px] px-4 md:h-[30px] rounded-md flex items-center justify-center gap-2 overflow-hidden relative transition-all duration-500 cursor-pointer">
              ...
              {current.faqs.map((faq, index) => (
                ...
                >
                  <button
                    ...
                    className="w-full flex justify-between items-center text-left px-8 py-5 text-base md:text-[22px]"
                  >
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-500 w-full max-w-[90%] ${
                      activeIndex === index ? "max-h-40 px-8 pb-5" : "max-h-0"
                    }`}
                  >
                    <p className="text-[#A1A1A1] text-base leading-[27px]">
                      {faq.answer}
